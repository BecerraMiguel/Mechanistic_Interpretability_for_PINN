...
class TestGridGeneration:
    ...
    def test_generate_grid_1d(self):
        ...
        store = ActivationStore("dummy.h5")
        grid = store._generate_grid(resolution=50, domain_bounds=((0.0, 2.0),))
    ...
    def test_generate_grid_3d(self):
        ...
        grid = store._generate_grid(
            resolution=5, domain_bounds=((0.0, 1.0), (-1.0, 1.0), (0.0, 2.0))
        )
...
class TestActivationExtraction:
    ...
    def simple_model(self):
        ...
        model = MLP(input_dim=2, output_dim=1, hidden_dims=[16, 16], activation="tanh")
        model.eval()
        return model
    ...
    def test_extract_on_grid_saves_correct_structure(self, simple_model, tmp_path):
        ...
        store.extract_on_grid(
            simple_model, grid_resolution=10, domain_bounds=((0.0, 1.0), (0.0, 1.0))
        )
...
class TestLoadingData:
    ...
    def populated_store(self, tmp_path):
        ...
        model = MLP(input_dim=2, output_dim=1, hidden_dims=[8, 8], activation="tanh")
        model.eval()
...
class TestVisualization:
    ...
    def populated_store(self, tmp_path):
        """Create a store with extracted activations."""
        model = MLP(input_dim=2, output_dim=1, hidden_dims=[8], activation="tanh")
        model.eval()
    ...
    def test_visualize_layer_summary_save(self, populated_store, tmp_path):
        ...
        fig = populated_store.visualize_layer_summary(
            "layer_0", n_neurons=8, save_path=str(save_path)
        )
...
class TestIntegration:
    ...
    def test_workflow_extract_load_visualize(self, tmp_path):
        ...
        # 1. Create model
        model = MLP(input_dim=2, output_dim=1, hidden_dims=[8], activation="relu")
        model.eval()
